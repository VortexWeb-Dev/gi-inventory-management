...
//   useEffect(() => {
//     console.log("from area: ", fromArea);
//     console.log("to area: ", toArea);
//   }, [fromArea, toArea]);
...
const FilterBar = ({ filteredData, setFilteredData, setRefresh, fullPriceRange, fullAreaRange}) => {
  const savedFilteredData = useRef(null);
  const hasRun = useRef(false);
  ...
  useEffect(() => {
    if (filteredData && filteredData.length > 0) {
      const { priceRange: newPriceRange, areaRange: newAreaRange } =
        calculateRanges("filtered",filteredData);
  ...
  const handleReset = () => {
    ...
    setFromPrice(priceRange[0]);
    setToPrice(priceRange[1]);
    setFromArea(areaRange[0]);
    setToArea(areaRange[1]);
