...
const PropertyBrochureGenerator = ({ listing }) => {
  ...
  const generatePDF = async () => {
    ...
    try {
      ...
      const priceWidth = doc.getStringUnitWidth(priceText) * doc.internal.getFontSize() / doc.internal.scaleFactor;
      ...
      if (listing.images && listing.images.length > 0) {
        try {
          ...
          if (mainImageBase64) {
            ...
            doc.text('Image not available', pageWidth / 2, yPos + (contentWidth * 0.3), { align: 'center' });
      ...
      doc.text(listing.locationBayut || 'Location information not available', margin, yPos);
      ...
      doc.setFontSize(14);
      doc.setFont('helvetica', 'bold');
      doc.text(String(listing.bedrooms || 'N/A'), margin + detailsWidth * 0.2, yPos);
      doc.setFontSize(10);
      doc.setFont('helvetica', 'normal');
      ...
      doc.setFontSize(14);
      doc.setFont('helvetica', 'bold');
      doc.text(String(listing.bathrooms || 'N/A'), margin + detailsWidth * 1.2, yPos);
      doc.setFontSize(10);
      doc.setFont('helvetica', 'normal');
      ...
      doc.setFontSize(14);
      doc.setFont('helvetica', 'bold');
      doc.text(`${listing.size || 'N/A'} sq.ft`, margin + detailsWidth * 3.2, yPos);
      doc.setFontSize(10);
      doc.setFont('helvetica', 'normal');
      ...
      doc.setFontSize(10);
      doc.setFont('helvetica', 'bold');
      doc.text(`Agent: ${listing.agentName || 'N/A'}`, pageWidth - margin - 50, footerY);
      doc.setFont('helvetica', 'normal');
      doc.text(`Owner: ${listing.ownerName || 'N/A'}`, pageWidth - margin - 50, footerY + 5);
      ...
      if (listing.images && listing.images.length > 1) {
        ...
        const imagesToProcess = listing.images.slice(1, Math.min(listing.images.length, 9));
        const imagePromises = imagesToProcess.map(image => getBase64FromUrl(image.url));
        ...
        for (let i = 0; i < imageResults.length; i++) {
          ...
          if (result.status === 'fulfilled' && result.value) {
            try {
              ...
              doc.setTextColor(100, 100, 100);
              doc.text('Image not available', imgX + galleryImageWidth/2, imgY + galleryImageHeight/2, { align: 'center' });
            }
          } else {
            // Add placeholder for failed image
            doc.setFillColor(200, 200, 200);
        ...
        doc.setFontSize(10);
        doc.setFont('helvetica', 'bold');
        doc.text(`Agent: ${listing.agentName || 'N/A'}`, pageWidth - margin - 50, galleryFooterY);
        doc.setFont('helvetica', 'normal');
        doc.text(`Owner: ${listing.ownerName || 'N/A'}`, pageWidth - margin - 50, galleryFooterY + 5);
